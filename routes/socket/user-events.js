...
const updateSeatedUser = (socket, passport, data) => {
	...
	Account.findOne({ username: passport.user }).then(account => {
		...
		const isRainbowSafe = !game.general.rainbowgame || (game.general.rainbowgame && account.wins + account.losses > 2);
		const isPrivateSafe =
...
module.exports.handleAddNewGame = (socket, passport, data) => {
	...
	const newGame = {
		...
		general: {
			...
			rainbowgame: user.wins + user.losses > 2 ? data.rainbowgame : false,
